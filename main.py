...
def Details() :
    ...
    if request.method == 'POST' and 'Username' in request.form and 'aptNo' in request.form and 'TFatherName' in request.form and 'PerAddr' in request.form :
        ...
        cursor.execute('SELECT T_ID, Fname FROM TENANT WHERE EMAIL= % s',(Uname,))
        mysql.connection.commit()
        tid_list1 = cursor.fetchone()
        t_id = tid_list1['T_ID']
        fname = tid_list1['Fname']
        ...
        if t_id != None and res1 != None :
            ...
            Deposit = rentAmt * 2
            return redirect(url_for('Contract', aptNo=aptNo ,Tname=Tname,Uname=Uname, TFatherName=TFatherName, PAddress=PAddress, Date=Date, rentAmt=rentAmt, Deposit=Deposit))
        else :
...
def alreadyTenant() :
    ...
    if request.method == 'POST' and 'Username' in request.form and 'aptNo' in request.form :
        ...
        if t_id != None and res1 != None :
            ...
            late_fee = late_fee
            totAmt = int(rentAmt) + int(late_fee)
            return redirect(url_for('Payment', aptNo=aptNo ,Tname=Tname, Uname=Uname,PhNo=PhNo , rentAmt=rentAmt, late_fee=late_fee, totAmt=totAmt))
        else :
            Error = 'Invalid Username or Apartment No.!!'
    elif request.method == 'POST' :
        Error= 'Please fill out the form!'
...
from cosine_search.cosine_matcher import match_apartments

@app.route('/search_apartments', methods=['POST'])
def search_apartments():
    try:
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute("SELECT * FROM apartment")  # Check your actual table name
        apartments = cursor.fetchall()
        cursor.close()

        matches = match_apartments(query, apartments)
        ...
        return jsonify([
            {
            "APT_TITLE": "Test Apartment",
            "ROOM_NO": "101",
            "RENT_PER_MONTH": "15000",
            "AREA": "850",
            "APARTMENT_DESC": "Test Description" 
            }
        ])

@app.route('/Details/<room_no>')
def apartment_details(room_no):
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute("SELECT * FROM apartment WHERE ROOM_NO = %s", [room_no])
...
def smartsearch():
    try:
        ...
        matched_apartments = match_apartments(query, apartments)

        internet_required = filters.get("internet", False)
        parking_required = filters.get("parking", False)

        filtered_apartments = []
        for apt in matched_apartments:
            if internet_required and apt['INTERNET'].lower() != 'yes':
                continue
            if parking_required and apt['PARKING'].lower() != 'yes':
                continue
            apt['TOTAL_ROOMS'] = apt.get('BEDROOM', 0) + apt.get('KITCHEN', 0) + apt.get('LIVING_ROOM', 0)
...
if os.path:
    try:
        pathname = 'static/' + os.path['PATHNAME']
        os.remove(pathname + os.path['PHOTO1'])
        os.remove(pathname + os.path['PHOTO2'])
